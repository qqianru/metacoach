...
- **练一题 (practice)** — 对话式陪练，教练会根据学生状态给提示、降情绪、防走神。
- **考试模式 (exam)** — 教练全程沉默，只记录过程；学生点"提交答案"后才生成复盘。
- **错题手术 (micro_surgery)** — 结构化三步流程，把一道错题变成一条可执行的"炒菜程序"：
  1. **定位病灶** —— 错在哪个具体的概念/公式上？（不是"粗心"，而是"我不知道这里要加辅助线构造全等三角形"）
  2. **寻找线索** —— 题目里哪个词/条件，本来应该触发我用这个方法？（如"看到'中点'，就该想到倍长中线"）
  3. **写下炒菜程序** —— 总结成一条"如果…就…"的口诀

  每一步都可以点"让教练打磨"获得 good / needs_work 的反馈；三步写完后点"生成我的错题手术卡"，会输出一张带标题、知识点标签、病灶、触发词、口诀、升华、打磨评分的结构化卡片，自动保存到数据库供老师端查看。
